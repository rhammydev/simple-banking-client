...
export function AccountDetailPage() {
  ...
  if (isError || !account) {
    return (
      <ErrorState
        message={(error as ApiError | null)?.message ?? "Account not found."}
      />
    );
  }
  ...
  return (
    <div className="flex flex-col gap-8">
      ...
      <Link
        to="/accounts/$accountNumber/statement"
        params={{ accountNumber }}
        className="-mt-2 inline-flex items-center gap-1.5 self-start text-xs font-semibold uppercase tracking-wide text-navy hover:text-gold"
      >
      ...
      <Modal
        title={modalTitles.transfer}
        isOpen={activeModal === "transfer"}
        onClose={closeModal}
      >
        <TransferForm
          defaultSenderAccountNumber={accountNumber}
          onDone={closeModal}
        />
      </Modal>
      ...
      >
        ...
        <Button
          ...
          onClick={() =>
            reactivateAccount.mutate(accountNumber, { onSuccess: closeModal })
          }
        >
      ...
      >
        <p className="text-sm text-ink-soft">
          ...
          . This action can be reversed later from the Reactivate action.
        </p>
        <Button
          variant="danger"
          className="mt-5 w-full"
          ...
          onClick={() =>
            deleteAccount.mutate(accountNumber, {
